...
public class InventoryManager {
    ...
    /**
     * Updated to return boolean so main class can detect if file loading failed.
    ...
    /**
     *  Method to sort the existing Linked List by Brand.
     */
    public void generateReportByBrand() {
        if (head == null) {
            System.out.println("Inventory is empty.");
            return;
        }

        //  Collect all data from the Doubly Linked List into a temporary list
        ArrayList<Motorcycle> brandList = new ArrayList<>();
        Node current = head;
        while (current != null) {
            // Skip the header row if it accidentally got into your list
            if (!current.data.engineNumber.equalsIgnoreCase("Engine Number")) {
                brandList.add(current.data);
            }
            current = current.next;
        }

        // Sort the list by Brand name
        Collections.sort(brandList, new Comparator<Motorcycle>() {
            @Override
            public int compare(Motorcycle m1, Motorcycle m2) {
                return m1.brand.compareToIgnoreCase(m2.brand);
            }
        });

        //  Print the sorted results
        System.out.println("\n--- Inventory Report (Sorted by Brand) ---");
        for (Motorcycle m : brandList) {
